...
2. `breaker.beforeRequest(endpointKey)` gates the call. If the circuit
   is open, the endpoint is skipped (no upstream call wasted) and a
   failover is counted.
...
- `failoverAttempts` - bumps once per additional attempt against a
  different endpoint. A single-attempt success records zero.
